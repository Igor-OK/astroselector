...
var ModelH = {
  ...
  native_lattitude: 53.9, //  LATITUDE  think about (and CHECK) situation of birth, when beyong polar round (murmansk for example)
  ...
  console.log ('В Беларуси в это время разница от GMT - ',this.native_gmt);
  ...
  var st = _start[0] * 60; //So, getting in hours, transformin minutes
...
//todo to know about e
    ...
    // 2 intervals of cuspids - Найдите следующие интервалы куспидов домов
    var h11 = RAMC + 30;
    var h12 = RAMC + 60;
    var h2 = RAMC + 120;
    var h3 = RAMC + 150;

    // 3 proportion ofa semi-arcs -  пропорции полудуг
    var f11 = 1/3;
    var f12 = 2/3;
    var f2 = 2/3;
    var f3 = 1/3;

    // 4 Cuspal declination - склонения куспидов В РАДИАНАХ , для градусов - *180/Math.PI
    var d11 = Math.asin(sin_e * Math.sin(h11/180*Math.PI)); 
    var d12 = Math.asin(sin_e * Math.sin(h12/180*Math.PI)); 
    var d2 = Math.asin(sin_e * Math.sin(h2/180*Math.PI)); 
    var d3 = Math.asin(sin_e * Math.sin(h3/180*Math.PI)); 
    ...
    function Placidus_iteration(d11, d12, d2, d3){ //arguments in RADIAN
      ...
      // 5 First Intermediate values = первые промежуточные величины
      const a11 = funcA(f11, d11);
      const a12 = funcA(f12, d12);
      const a2 = funcA(f2, d2);
      const a3 = funcA(f3, d3);

      // 6 House cusp positions -  положения куспидов домов 
      const m11 = funcM(a11, h11, d11);
      const m12 = funcM(a12, h12, d12);
      const m2 = funcM(a2, h2, d2);
      const m3 = funcM(a3, h3, d3);

      // 7 Intermediate house cusps -  промежуточные вычисления куспидов домов
      const r11 = funcR(h11, m11);
      const r12 = funcR(h12, m12);
